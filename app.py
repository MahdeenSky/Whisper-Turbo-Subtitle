...
def whisper_subtitle(uploaded_file, Source_Language, translate=False, device="cuda", compute_type="float16"):
    ...
    if global_model is None:
        ...
        global_model = whisperx.load_model("large-v2", device, compute_type=compute_type)
    model = global_model

    audio = whisperx.load_audio(uploaded_file)
    task = "translate" if translate else "transcribe"
    result = model.transcribe(audio, batch_size=32 if device == "cuda" else 1, task=task, language=language_dict[Source_Language]['lang_code'] if Source_Language != "Automatic" else None)
    ...
    if global_align_model is None:
        ...
        global_align_model, metadata = whisperx.load_align_model(language_code=result["language"], device=device)
...
def subtitle_maker(Audio_or_Video_File, Link, File_Path, Source_Language, translate, device, compute_type):
    ...
    try:
        srt_path, txt_path, beep_audio_path = whisper_subtitle(Audio_or_Video_File, Source_Language, translate=translate, device=device, compute_type=compute_type)
...
def main(debug, share, device, compute_type):
    ...
    demo = gr.Interface(
        fn=lambda *args, **kwargs: subtitle_maker(*args, device=device, compute_type=compute_type, **kwargs),
